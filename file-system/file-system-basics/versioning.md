...
We are using a temporal DAG with fuctional persistence. Unlike Git, it is curretly designed to be ”single writer” — new patches must be applied in order, and conflicts are not automatically resolvable. That is on the roadmap, but there’s a lot of bootstrap in the meantime.
...
There is one diff here: `<A’, C’, E’>`. The rest is shard structurally. Thus, `B` originated in Generation 0, but is a member of both Generations 0 and 1.
